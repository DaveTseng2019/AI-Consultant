...
pub async fn provider_open(
    ...
    // Inbound-hint transport: the document.title codec. Cross-platform via Tauri's
...
pub async fn provider_open_login(
    ...
    let already_there = provider_webview
        .url()
        .map(|current| current == login_url)
        .unwrap_or(false);
    if already_there {
        // notes: grok and gemini declare their app URL as their login URL, so on those two this
        //        button asked the page to go where it already was and the user saw nothing happen:
        //        no fresh document, and for Grok no fresh document title, which is the single event
        //        its readiness rides on. Reload instead, down the same path provider_reload takes so
        //        Grok's title signal is carried across the new epoch. This branch stops being
        //        reachable if those adapters ever gain a distinct login URL.
        let prepared_epoch = prepare_grok_navigation(&provider)?;
...
mod tests {
    ...
    use super::{
        ...
        claim_grok_popup_recovery, confirm_grok_page_load, decide_new_window_action,
        delayed_grok_bridge_script, eval_callback_reports_true, expire_grok_navigation_start_lease,
        fresh_session_boot, gemini_sorry_navigation_active, generic_staleness_dispatch_allowed,
        grok_app_title_ready, grok_app_title_signal_is_current, grok_auth_popup_may_complete_login,
        ...
        grok_popup_recovery_claim_is_current, grok_popup_recovery_needed, grok_reconnect_allowed,
        grok_stale_reason, in_page_new_chat_selector, new_session_paths,
