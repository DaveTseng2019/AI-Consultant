...
New providers are not adapter-only in v1. The fixed provider set is `chatgpt`, `claude`, `gemini`, and `grok`; adding another provider requires code and UI changes, not just a new adapter JSON.
