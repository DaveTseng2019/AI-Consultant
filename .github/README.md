...
|---|---|---|
| [v0.0.14](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.14) | 2026-09-01 | The window title says what the app is doing: while it connects to the providers or runs a workflow, the current step takes the title over from the app name, so it stays readable in the taskbar, and the name comes back once nothing is running; Settings gains Start maximized, which takes effect at the next launch; and a dev build now carries a DEV suffix in its title and can run alongside an installed one, where launching it used to raise the installed window and exit instead |
