...
|---|---|---|
| [v0.0.17](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.17) | 2026-09-06 | The Linux build gets the layout every other platform already has: the provider panes used to be stacked into horizontal bands down the window, so the stage, resizing and parking the providers you are not reading all did nothing - the position and size the app asked for were discarded on Linux. Both are honoured now, and the stage, window resizing and typing inside a provider behave as they do elsewhere. Linux only; Windows and macOS are unchanged |
| [v0.0.16](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.16) | 2026-09-02 | The download button inside a provider pane now does something: no webview accepted a download before, so pressing Claude's Download, or ChatGPT's Download PNG, was silently ignored. The file is saved where the browser would have put it, and a notice names the path it went to. A text file that arrives this way is also read back and filed in the conversation under the provider and the file name, so a provider that answers by writing a document reaches the exported .md instead of leaving a one-line summary behind; an image or an archive is reported by path alone. This also fixes a document being captured twice in v0.0.15 |
| [v0.0.15](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.15) | 2026-09-01 | New conversation now works in the middle of a run: the button stays enabled, and pressing it stops the running question before opening the clean conversation, so a typo can be re-asked in one step instead of cancelling first; cancelling no longer leaves an "Error: Workflow cancelled by user" message behind, which used to arrive late and land in the new, unrelated conversation; and the provider pages switch to a new chat the moment you press it rather than at your next send, with a send during the reload waiting for them instead of dropping them from the run |
| [v0.0.14](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.14) | 2026-09-01 | The startup notice for a run that was killed mid-flight no longer points Replay at whatever that workflow ran last time - such a run leaves no recording of its own, so the button is now absent rather than replaying the wrong thing; the window title says what the app is doing: while it connects to the providers or runs a workflow, the current step takes the title over from the app name, so it stays readable in the taskbar, and the name comes back once nothing is running; Settings gains Start maximized, which takes effect at the next launch; and a dev build now carries a DEV suffix in its title and can run alongside an installed one, where launching it used to raise the installed window and exit instead |
| [v0.0.13](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.13) | 2026-08-31 | The portable build updates itself: press Download and update in Settings and the app closes, fetches the new release, replaces its own folder and reopens - no browser, no unzipping over it by hand, and a failure puts the old build back and opens update-log.txt to say where it stopped; the folder inside the zip also drops its version number, so an update replaces the install instead of landing a copy beside it (anyone on the v0.0.12 portable build still has to update by hand once - that build has no such button); New conversation no longer throws the mode away, so the conversation it opens is in the mode you were working in |
