...
| 版本 | 日期 | 異動 |
|---|---|---|
| [v0.0.12](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.12) | 2026-08-31 | 左邊的對話紀錄可以拖曳調整寬度，長標題看得完整，不用再靠猜；拉寬吃掉的是旁邊 provider 舞台的空間而不是右邊的對話欄，而且永遠留 240px 給舞台；上方那排操作模式只在對話還空著的時候出現，第一則訊息送出去就收起來，模式改看對話欄標題的徽章，要換模式按「新對話」；舞台的放大鈕現在任何狀態按一次就蓋掉模式列，以前在「未登入自動放大」狀態要按兩次——那正是第一次開程式登入四家 provider 時每一家的狀態 |
