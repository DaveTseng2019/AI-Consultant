...
| 版本 | 日期 | 異動 |
|---|---|---|
| [v0.0.17](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.17) | 2026-09-06 | Linux 版的版面終於跟 Windows、macOS 一樣：以前各家 AI 的畫面在 Linux 上會被由上而下切成一條一條的橫帶，中央舞台、縮放、把沒在看的那幾家收起來全都沒有作用，因為程式給的位置與大小在 Linux 上被整個忽略。現在位置與大小都算數，中央舞台、視窗縮放、在 AI 畫面裡打字都跟其他平台一樣。只動到 Linux，Windows 與 macOS 沒有任何改變 |
| [v0.0.16](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.16) | 2026-09-02 | AI 畫面裡的下載按鈕終於有反應了：以前沒有任何一個 webview 接受下載，所以按 Claude 的 Download、ChatGPT 的 Download PNG 都是按了沒事。現在檔案會存到瀏覽器本來就會存的位置，並顯示一列訊息告訴你存到哪裡。存下來的如果是文字檔，內容還會被讀回來、以「AI 名稱 · 檔名」加進對話裡，所以像 Claude 那種「寫一份文件給你」的回答不會只剩一句摘要，整份文件會進到匯出的 .md；圖片或壓縮檔則只報路徑。同時修正 v0.0.15 會把同一份文件抓兩次的問題 |
| [v0.0.15](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.15) | 2026-09-01 | 執行中也能按「新對話」了：那顆按鈕不再變灰，按下去會先停掉正在跑的那一題再開新對話，問錯或打錯字想重問不必先按取消；主動取消不再留下 Error: Workflow cancelled by user 那行紅字（它會比畫面清空還晚到，落在剛開的新對話裡）；按下新對話時各家 AI 的畫面立刻換成新對話，不再等到下一次送出才清，換頁還沒完成就送出的話會等它們準備好，不會漏掉任何一家 |
| [v0.0.14](https://github.com/DaveTseng2019/AI-Consultant/releases/tag/v0.0.14) | 2026-09-01 | 上一次執行沒跑完就被關掉時，重新開啟後的提示不再把「重播」指到同一個流程上一次跑完的那一筆——被直接關掉的那一次其實沒有留下紀錄，現在就不給那顆按鈕，而不是給一顆會跑錯東西的；視窗標題會說程式在做什麼：連線 AI 或跑工作流程的時候，標題的名稱暫時被目前那一步取代，縮到工作列也看得到，閒著才寫回 AI Consultant；設定裡多了「啟動時視窗最大化」，改了下次啟動生效；開發版的標題尾巴多一個 DEV，而且開發版與正式版現在可以同時開著（以前正式版開著時啟動開發版，會變成把正式版叫到前面然後自己結束） |
