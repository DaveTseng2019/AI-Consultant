...
標題列另有一顆**收合**鍵：按下去 provider 只剩圖示與名稱那一列，真實網頁也跟著收掉（webview 被
park 到畫面外），空間讓給「流程追蹤」與「AI 連線」。置中別的 provider 會自動展開。收合狀態跟放大
一樣不寫進設定。

**「⋯」只在文字檢視顯示。** 那個下拉是 HTML，在真實網頁模式會被原生 webview 蓋住——開了也看不見。
重新載入與回報要在文字檢視下用。

手動按**放大**時，只要已經提過問題或畫面上有對話紀錄，上方的「傳送給已選的 AI」那一列也會讓出來，
舞台頂到欄位最上面。按「還原」就回來。「無法送出」的提示是例外，會留著。
...
- **重播**：**沿原路重跑一次**，AI 會重新回答，不是回放舊畫面。重跑前有五道把關：
  ...
  擋下時會說明是哪一種。
- **圖表**：Mermaid 圖的原始碼在 DOM 裡通常找不到——四家各自畫成圖片、SVG 或擺在旁邊的檔案裡。
  抓取時會分別去各家真正存原始碼的地方取（ChatGPT 與 Grok 的 React props、Gemini 的開頭關鍵字、
  Claude 的回答元素），讀不到就丟掉那個區塊，不會把按鈕文字當成圖表匯出。
- **AI 交給你的檔案**：按 provider 自己的下載按鈕，檔案會存到瀏覽器本來就會存的位置，
  並顯示存檔路徑。文字檔還會被讀回來，以「AI 名稱 · 檔名」加進對話（上限 512 KB、只收有效 UTF-8）；
  圖片或壓縮檔只報路徑。
- **匯出**：Markdown、HTML，或跑你在設定裡指定的自訂腳本。
  檔名是「日期 + 對話標題」，日期取對話開始那一天。
  HTML 還會把表格畫成表格：終端機的框線表格、被壓成一行的 Markdown 表格、成串的「項目：值」，都會先還原成列再渲染。
