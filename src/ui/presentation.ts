...
/**
 * What the stage should look like for the provider being centred, or `undefined` while its login is
 * still unreported. Undefined means "not yet" rather than "leave it": at startup every provider
 * reads `unknown`, and deciding then would expand the stage for an account that is in fact signed
 * in. The caller waits for the first real report and decides once.
 */
export function centerStageDecision(
  login: ProviderState['login'] | undefined,
  remembered: CenterSurface,
): { stageExpanded: boolean; surface: CenterSurface } | undefined {
  if (login === undefined || login === 'unknown') return undefined;
  const signedOut = login !== 'logged_in';
  return { stageExpanded: signedOut, surface: signedOut ? 'native' : remembered };
}
