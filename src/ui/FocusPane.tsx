...
function FirstRunPanel({
  ...
  return (
    ...
    <section
      ...
      className="grid min-h-[160px] flex-1 place-items-center overflow-auto rounded-lg border border-zinc-200 bg-gradient-to-b from-sky-50 to-white p-4 dark:border-zinc-800 dark:from-sky-950/30 dark:to-zinc-950"
    >
        ...
        <div className="flex flex-wrap justify-end gap-2 text-xs">
          ...
          {onToggleStageExpanded ? (
            <button
              ...
              aria-label={stageExpanded ? t('provider.collapseStage') : t('provider.expandStage')}
              title={stageExpanded ? t('provider.collapseStage') : t('provider.expandStage')}
              onClick={onToggleStageExpanded}
            >
              {/* Same corner-arrow pair the transcript maximise button uses, so one icon means one
                  thing across the app. The word moves to aria-label/title rather than disappearing. */}
              {stageExpanded ? (
