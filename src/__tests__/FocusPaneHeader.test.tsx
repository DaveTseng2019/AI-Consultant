...
describe('FocusPane provider header', () => {
  it('renders the login CTA only when the focused provider needs login', () => {
    // 用 key 取字，不寫死字面：這個測試守的是「只有需要登入時才出現這顆鈕」，不是它叫什麼名字。
    const label = t('provider.login', 'en');
    expect(renderHeader('logged_out')).toContain(label);
  ...
  it('keeps the connection strip while the stage is expanded so the user can still switch AI', () => {
