...
describe('injected engine input hardening', () => {
  ...
  it('keeps Gemini sendable while signed out, because that page still answers', () => {
    // Detecting the absent Google session is easy and wrong: login gates isSendable, so a
    // logged_out Gemini is dropped from every fan-out even though the signed-out page works.
    const env = createEnv({ inputKind: 'contenteditable' });
    env.detectorElements.set('a[href*="accounts.google.com/ServiceLogin"]', [
      new FakeElement(env.document, 'a', 'Sign in'),
    ]);
    env.detectorElements.set('.ql-editor[contenteditable="true"]', [new FakeElement(env.document, 'div', '')]);
    return installEngine(env).then((handler) => {
      dispatchAdapter(handler, {
        provider: 'gemini',
        loginDetectors: ['.ql-editor[contenteditable="true"]'],
        loggedOutDetectors: [],
      });
