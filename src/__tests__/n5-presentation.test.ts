...
describe('N5 webview presentation model', () => {
  ...
  it('remembers the centre stage surface across a settings round-trip and rejects junk', () => {
    // The stage reopens on the face the user last chose, so the choice has to survive a restart.
    expect(normalizeSettings({}).centerSurface).toBe('text');
    expect(mergeSettings({}, { centerSurface: 'native' }).centerSurface).toBe('native');
    expect(normalizeSettings({ centerSurface: 'native' }).centerSurface).toBe('native');
    expect(normalizeSettings({ centerSurface: 'bad' }).centerSurface).toBe('text');
  });

  it('waits for a real login report before sizing the stage, then follows it', () => {
    // 'unknown' is every provider at startup. Deciding then would expand the stage for an account
    // that is in fact signed in, which is what the user must never see after a restart.
    expect(centerStageDecision('unknown', 'text')).toBeUndefined();
    expect(centerStageDecision(undefined, 'text')).toBeUndefined();

    // Signed out needs the whole pane and the real page; the remembered face does not apply.
    expect(centerStageDecision('logged_out', 'text')).toEqual({ stageExpanded: true, surface: 'native' });
    expect(centerStageDecision('blocked', 'native')).toEqual({ stageExpanded: true, surface: 'native' });

    // Signed in goes back to normal size and to whichever face was remembered.
    expect(centerStageDecision('logged_in', 'text')).toEqual({ stageExpanded: false, surface: 'text' });
    expect(centerStageDecision('logged_in', 'native')).toEqual({ stageExpanded: false, surface: 'native' });
  });
