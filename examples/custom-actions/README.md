...
後兩者要**先問完一題**，按鈕才按得下去。「本次執行紀錄」還多一個條件：那份紀錄只活在記憶體裡，
所以從左邊紀錄還原一則舊對話、或重開 app 之後，按鈕會是灰的——滑鼠滑過去會說原因。
重問一次就會亮回來。
...
**`-MarkdownPath`**：跟「匯出 .md」同一份內容，也寫在同一個地方——「匯出 .md」上次存檔的
資料夾，第一次用則是 `文件\AI Consultant 匯出\`。檔名蓋的是**對話開始的時間**，不是按下按鈕的
時間，所以同一則對話按幾次都是覆寫同一個檔，不會累積一堆只差在時間戳的副本。不用自己
`Copy-Item` 留一份，這個檔就是要留著的那份。
